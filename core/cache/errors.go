...
var (
	...
	ErrInvalidTargetType error = errors.New("target must be a byte slice for pure gzip decoding")

	// ErrInvalidPayloadType is returned when the payload type is not compatible with the encoder
	ErrInvalidPayloadType error = errors.New("payload must be a byte slice for pure gzip encoding")

	// ErrClosed is returned when attempting to use a closed cache
	ErrClosed error = errors.New("cache is closed")
)
