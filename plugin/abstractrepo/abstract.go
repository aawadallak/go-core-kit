...
func (r *AbstractRepository[T]) Save(ctx context.Context, target *T) (*T, error) {
	...
	if r.opts.NoReturn {
		tx = tx.Clauses(clause.Returning{Columns: nil})
	}
...
func (r *AbstractRepository[T]) FindOne(ctx context.Context, filter *T) (*T, error) {
	...
	for _, preload := range r.opts.Preloads {
	...
	if err := tx.
		...
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, common.NewErrResourceNotFound(
				fmt.Errorf("resource %s not found on database", reflect.TypeFor[T]().Name()),
			)
		}
...
func (r *AbstractRepository[T]) Find(ctx context.Context, filter *T) ([]*T, error) {
	...
	if err := tx.WithContext(ctx).Where(filter).Find(&out).Error; err != nil {
		// TODO: VALIDATE IF THIS IS THE CORRECT WAY TO HANDLE THIS ERROR
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, common.NewErrResourceNotFound(
				fmt.Errorf("resource %s not found on database", reflect.TypeFor[T]().Name()),
			)
